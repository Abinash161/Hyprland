...
nl -ba -w1 -s $'\t' "$TMP_ORIG" | while IFS=$'\t' read -r idx entry; do
  # sanitize into UTF-8, drop NULs, remove control chars, collapse whitespace and truncate to 200 chars
  preview=$(printf '%s' "$entry" | tr -d '\000' | iconv -c -f utf-8 -t utf-8 2>/dev/null \
    | perl -pe 's/[\x00-\x09\x0B\x0C\x0E-\x1F]//g' \
    | tr '\n' ' ' | tr -s ' ' | cut -c1-200)
  # escape characters that can break Pango/GTK markup in wofi/rofi
  preview=$(printf '%s' "$preview" | perl -pe 's/&/&amp;/g; s/</&lt;/g; s/>/&gt;/g')
