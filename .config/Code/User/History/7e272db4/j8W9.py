...
def make_network_ui(Gtk, Gdk):
    ...
    # Keep pin state on the window so main() can read it.
    pin_btn.connect("toggled", lambda b: setattr(win, "_pinned", b.get_active()))
    ...
    try:
        ...
        for ln in lines:
            ...
            # allow the label to ellipsize if too long
            ...
            def make_connect_cb(ss, security):
                def cb(*args):
                    # If network is protected, ask for a password first
                    needs_pass = security and security != "--"
                    pwd = None
                    if needs_pass:
                        # simple password dialog
                        dlg = Gtk.Dialog(title=f"Password for {ss}", transient_for=win, flags=0)
                        ...
                        dlg.destroy()
                        if not pwd:
                            # user canceled or empty password; open editor as fallback
                            run_detached(["nm-connection-editor"])
                            Gtk.main_quit()
                            return
                    ...
                    try:
                        cmd = ["nmcli", "device", "wifi", "connect", ss]
                        if pwd:
                            cmd += ["password", pwd]
                    ...
                    Gtk.main_quit()
                return cb
            btn.connect("clicked", make_connect_cb(ssid, security))
    ...
    top_margin = 36
    win.show_all()
    # If the caller supplied position coordinates, try to move the popup
    try:
...
def main():
    ...
    win.present()
    # Auto-close only via timeout when not pinned (avoids premature close when
    # popup is on top of another window). Use a conservative timeout (12s).
    try:
        def _timeout_close():
            if getattr(win, "_pinned", False):
                return True
            Gtk.main_quit()
            return False
        GLib.timeout_add_seconds(12, _timeout_close)
    except Exception:
        pass
